...
void nms(std::vector<std::vector<float>>& boxes, const float iou_threshold)
{
    ...
    //std::remove_if(boxes, [](const std::vector<float>& box) { return box[4] == 0.f; });
    boxes.erase(std::remove_if(boxes.begin(), boxes.end(),
             [](const std::vector<float>& box) {
                 // Make sure the box has at least 5 elements to avoid out-of-bounds access
                 return box.size() >= 5 && box[4] == 0.f;
             }),
             boxes.end());
}
